...
class DCRNNSupervisorTest:
    ...
    def load_model(self, filename):
        ...
        print("\n\nexpected\n", self.dcrnn_model.state_dict().keys())
    ...
    def predict_idx(self, idx, mask=False):
        with torch.no_grad():
            self.dcrnn_model = self.dcrnn_model.eval()

            test_loader = self._data['test_loader']

            x_i, y_i = test_loader.get_element(idx)
            x_i, y_i = self._prepare_data(x_i, y_i)
            #print(x_i.shape)
            output = self.dcrnn_model(x_i) # [num_timesteps_out, batch_size, num_nodes]
            #output = torch.squeeze(output, dim=2)
            if mask:
                output = output[:, :, self._active_nodes == 1]
            return output
    ...
    def tune_model(self, x, y, epochs=1, lr=0.01):
        epsilon = 1e-8
        optimizer = torch.optim.Adam(self.dcrnn_model.parameters(), lr=lr, eps=epsilon)
        for epoch_num in range(epochs):
            optimizer.zero_grad()
            self.dcrnn_model = self.dcrnn_model.train()
            output = self.dcrnn_model(x, y, 0)
            loss = self._compute_loss(y, output)
            loss.backward()

            optimizer.step()
    
    def tune_model_batch(self, batches, epochs=1, lr=0.01):
        epsilon = 1e-8
        optimizer = torch.optim.Adam(self.dcrnn_model.parameters(), lr=lr, eps=epsilon)
        for epoch_num in range(epochs):
            for (x, y) in batches:
