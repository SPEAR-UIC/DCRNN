...
class DataLoader(object):
    ...
    def get_element(self, index):
        """
        Get a specific test element at index i.
        """
        if index < 0 or index >= self.size:
            raise IndexError("Index out of range")

        # Get the start and end indices for the specified element
        start_ind = index
        end_ind = index + 1
...
def load_dataset(dataset_dir, batch_size, test_batch_size=None, originalTest=False, **kwargs):
    data = {}
    for category in ['train', 'val', 'test']:
        if category == 'test' and originalTest:
            cat_data = np.load(os.path.join(dataset_dir, 'test_original.npz'))
        else:
    ...
    scaler.fit(np.concatenate((np.reshape(data['x_test'], -1), np.reshape(data['y_test'], -1))))
